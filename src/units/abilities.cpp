...
bool attack_type::overwrite_special_checking(active_ability_list& overwriters, const active_ability& overwrited) const
{
	...
	for(const auto& j : overwriters) {
		...
		if(overwrite_specials) {
			...
			if((*overwrite_specials)["affect_side"].str("both") == "self" || (*overwrite_specials)["affect_side"].str("both") == "opponent") {
				if((*overwrite_specials)["affect_side"].str("both") == "self" && loc != self.loc) {
					continue;
				}
				if(other.un && (*overwrite_specials)["affect_side"].str("both") == "opponent" && loc != other.loc) {
					continue;
				}
			}
...
{
	// If ctx is not empty, then it is a special and the code is already called from get_specials_and_abilities() for [overwrite_specials] feature, and musn't to be called from here,
	// but only in cases where ctx is empty as for engine abilities like [leadership] or [heal] who don't be specials.
	if(!ctx) {
		edit_list(list);
	}
