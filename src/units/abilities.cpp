...
unit_ability_list attack_type::overwrite_special_checking(const std::string& ability, unit_ability_list temp_list, unit_ability_list abil_list, const std::string& filter_self, bool is_special) const
{
	...
	for(const auto& i : abil_list) {
		...
		for(unit_ability_list::iterator j = temp_list.begin(); j != temp_list.end();) {
				...
				bool is_overwrited = temp_overwrite;
				if(affect_side){
					if(special_active_impl(shared_from_this(), other_attack_, *i.ability_cfg, AFFECT_SELF, ability, filter_self)){
						is_overwrited = temp_overwrite && special_active_impl(shared_from_this(), other_attack_, *j->ability_cfg, AFFECT_SELF, ability, filter_self);
					}
					else if(special_active_impl(other_attack_, shared_from_this(), *i.ability_cfg, AFFECT_OTHER, ability, filter_self)){
						is_overwrited = temp_overwrite && special_active_impl(other_attack_, shared_from_this(), *j->ability_cfg, AFFECT_OTHER, ability, filter_self);
					}
				}
				const config& overwrite_filter = (*i.ability_cfg).child("overwrite_filter");
				bool overwrite = is_overwrited;
				if(overwrite_filter){
					bool special_matches = self_ ? (*self_).ability_matches_filter((*j->ability_cfg), "", overwrite_filter) : false;
					overwrite = is_overwrited && special_matches;
				}
				if(overwrite) {
					j = temp_list.erase(j);
				} else {
