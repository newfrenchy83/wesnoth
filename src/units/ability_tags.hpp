...
struct ability_list_defines
{
	...
	ENUM_AND_ARRAY(heals, regenerate, resistance, leadership, skirmisher, illuminates, teleport, hides, dummy, attacks, berserk, chance_to_hit, damage, damage_type, parry, disable, drains, firststrike, heal_on_hit, petrifies, plague, poison, slow, swarm)

	static const std::set<std::string>& weapon_number_tags()
	{
		static std::set<std::string> tags{attacks, damage, parry, chance_to_hit, berserk, swarm, drains, heal_on_hit};
		return tags;
	}
	...
	{
		static std::set<std::string> tags{attacks, berserk, chance_to_hit, damage, damage_type, parry, disable, drains, firststrike, heal_on_hit, petrifies, plague, poison, slow, swarm};
		return tags;
	}
};
